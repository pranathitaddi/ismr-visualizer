...
function ShapleyPanel({
  shapleyData, features, monthLabel, year, isMobile,
}: {
  ...
  const entries = (Object.entries(shapleyData) as [keyof ShapleyData, number][])
            ...
            return (
              <div key={key} className="flex-shrink-0 bg-white rounded-lg border border-slate-100 px-3 py-2 min-w-[90px]">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs font-semibold tabular-nums" style={{ color: pos ? '#2563eb' : '#ea580c' }}>
                    {pos ? '+' : ''}{val.toFixed(2)}
                  </span>
                </div>
          ...
          return (
            <div key={key} className="bg-white rounded-md border border-slate-100 px-3 py-2.5">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs font-bold font-mono text-slate-500 uppercase">{key}</span>
                <span className="text-sm font-semibold tabular-nums" style={{ color: pos ? '#2563eb' : '#ea580c' }}>
                  {pos ? '+' : ''}{val.toFixed(3)}
                </span>
              </div>
              <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
...
function ParameterPanel({
  ...
  // On mobile, render as bottom expandable drawer-style panel (full width)
  if (isMobile) {
    return (
      <div className="w-full">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            {isExperimentMode && <span className="text-xs text-amber-600 bg-amber-100 px-2 py-0.5 rounded-full font-medium">Experiment</span>}
          </div>
          <div className="flex items-center gap-2">
            <button
            ...
            <button onClick={onToggleExpanded} className="p-1.5 rounded-lg bg-slate-100 text-slate-500">
              <svg width="14" height="14" viewBox="0 0 12 12" fill="none">
                <path d={isPanelExpanded ? 'M2 8L6 4L10 8' : 'M2 4L6 8L10 4'} stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
          </div>
        </div>
          ...
          <>
            <div className="grid grid-cols-2 gap-2">
              {features.map(f => {
                ...
                return (
                  <div key={f.id} className={`rounded-lg border px-3 py-2.5 transition-all
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {isExperimentMode && (
                          <button onClick={() => onToggleFix(f.id)} disabled={loading}
                            className="text-xs px-1 py-0.5 rounded border leading-none disabled:opacity-40"
                            style={f.fixed ? { borderColor: '#e2e8f0', color: '#94a3b8' } : { borderColor: '#fbbf24', backgroundColor: '#fef3c7', color: '#d97706' }}>
                            {f.fixed ? '🔒' : '🔓'}
                          </button>
                        )}
                      </div>
                    </div>
                    <input type="range" min={range.min} max={range.max} step={range.step} value={f.value}
            ...
            <div className="flex items-center gap-2 mt-3">
              ...
              <button onClick={onSubmit} disabled={loading || !isExperimentMode}
  ...
  if (!isPanelExpanded) {
    return (
      <div className="flex-shrink-0">
        <button onClick={onToggleExpanded} title="Expand parameters"
          ...
          <span className="text-xs font-bold tracking-widest uppercase leading-none select-none"
  ...
  return (
    <aside className="w-60 flex-shrink-0 flex flex-col gap-3">
      ...
      <button onClick={onToggleExperimentMode} disabled={loading}
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 px-3 sm:px-4 md:px-6 py-4 sm:py-6">
      ...
      <div className="max-w-[1400px] mx-auto space-y-4 sm:space-y-5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex border border-slate-200 rounded-lg overflow-hidden bg-white shadow-sm">
              {(['markers', 'shapley'] as const).map(mode => (
                <button key={mode} onClick={() => setVizMode(mode)} disabled={loading}
            ...
            {/* Links */}
            <select value={linkType} disabled={loading} onChange={e => setLinkType(e.target.value)}
        ...
        {isMobile && vizMode === 'shapley' && (
          <ShapleyPanel
            shapleyData={shapleyData} features={features}
            monthLabel={currentMonthFull} year={selectedYear} isMobile />
        )}
        ...
        <div className={`flex gap-3 sm:gap-4 items-start ${isMobile ? 'flex-col' : ''}`}>
          ...
          {!isMobile && (
            vizMode === 'shapley'
              ? <ShapleyPanel shapleyData={shapleyData} features={features}
                  monthLabel={currentMonthFull} year={selectedYear} isMobile={false} />
          ...
          <div className={`${isMobile ? 'w-full' : 'flex-1 min-w-0'}`}>
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-3 sm:p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {vizMode === 'shapley' && !loading && (
                    <span className="text-xs px-2 py-0.5 rounded-full bg-blue-50 border border-blue-100 text-blue-600 font-medium hidden sm:inline">
                      SHAP · dummy data
                    </span>
                  )}
                </div>
              ...
              <div className="mt-2.5 flex items-center justify-center gap-4 sm:gap-6 text-xs text-slate-400">
                {vizMode === 'markers' ? (
                  <>
                    <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-green-400" /><span>Positive Phase</span></div>
                    <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-red-400" /><span>Negative Phase</span></div>
                  </>
                ) : (
                  <>
                    <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-blue-400" /><span>Positive contribution</span></div>
                    <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-orange-400" /><span>Negative contribution</span></div>
          ...
          {/* Right — Parameter panel (desktop) or below map (mobile) */}
          {!isMobile && (
            <ParameterPanel
              features={features} isExperimentMode={isExperimentMode}
              isPanelExpanded={isPanelExpanded} isMobile={false}
              onFeatureChange={handleFeatureChange} onToggleFix={toggleFix}
              onToggleExperimentMode={toggleExperimentMode}
              onToggleExpanded={handlePanelExpandedToggle}
              onSubmit={handleSubmit} loading={loading} prediction={prediction} />
          )}
        </div>

        {/* Mobile parameter panel — below map */}
        {isMobile && (
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-3">
            <ParameterPanel
              features={features} isExperimentMode={isExperimentMode}
              isPanelExpanded={isPanelExpanded} isMobile
              onFeatureChange={handleFeatureChange} onToggleFix={toggleFix}
              onToggleExperimentMode={toggleExperimentMode}
              onToggleExpanded={handlePanelExpandedToggle}
              onSubmit={handleSubmit} loading={loading} prediction={prediction} />
